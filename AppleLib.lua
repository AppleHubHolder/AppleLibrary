				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Apple Lib")
							if fold then
							else
								makefolder("./Apple Lib")
							end
							fold = isfolder("./Apple Lib/Themes")
							if fold then
							else
								makefolder("./Apple Lib/Themes")
							end
							fold = isfolder("./Apple Lib/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./Apple Lib Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Apple Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Apple Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Apple Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Apple Lib" then
		library.WorkspaceName = convertfilename(windowName, "Apple Lib")
	end
	local AppleLibrary = Instance_new("ScreenGui")
	library.MainScreenGui, MainScreenGui = AppleLibrary, AppleLibrary
	...
	AppleLibrary.Name = "     "
	AppleLibrary.Parent = library.gui_parent
	AppleLibrary.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	AppleLibrary.DisplayOrder = 10
	AppleLibrary.ResetOnSpawn = false
	main.Name = "main"
	main.Parent = AppleLibrary
	...
	function windowFunctions:CreateTab(options, ...)
		options = (options and (type(options) == "string") and resolvevararg("Tab", options, ...)) or options or {
			Name = "Apple Style: Elite Lego Hax"
		}
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Apple Lib") then
						makefolder("./Apple Lib")
					end
					local common_string = "./Apple Lib/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Apple Lib/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://00000001",
			Flag = "__Designer.Background.ImageAssetID",
			Value = "rbxassetid://00000001",
